...
<body>
        ...
        <div class="cardx">
            <div class="head">
                ...
                <div>
                    ...
                    <div class="sublabel">Secure checkout • <?php echo htmlspecialchars($settings['display_name']); ?>
                    </div>
                </div>
            ...
            <div class="body">
                <div class="grid">
                    <div>
                        <div class="amountBox">
                            ...
                            <div>
                                ...
                                <div class="value">
                                    <?php echo number_format($grand_total, 2) . ' ' . $settings['currency'] ?></div>
                            </div>
                        </div>
                        <table class="summary">
                            <tr>
                                <td>Amount</td>
                                <td><?php echo number_format($transaction_amount, 2) . ' ' . $settings['currency'] ?>
                                </td>
                            </tr>
                            <tr>
                                <td>Fee (fixed)</td>
                            ...
                            <tr>
                                ...
                                <td><strong><?php echo number_format($grand_total, 2) . ' ' . $settings['currency'] ?></strong>
                                </td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="rightCol">
                        ...
                        <div class="alert alert-danger">Google Pay DIRECT requires an ECv2 Public Key. Add it in gateway
                            settings.
                        </div>
                        <?php else: ?>
                        <div id="gpay-button-container"></div>
                        <div class="helper">Use Google Pay to complete this payment. You’ll be redirected afterward.
                        </div>
    ...
    const POST_URL = "<?php echo pp_get_site_url() . '/pp-content/plugins/' . $plugin_info['plugin_dir'] . '/' . $plugin_slug . '/process-payment.php'; ?>";
    ...
    function onGooglePayLoaded() {
        ...
        client.isReadyToPay(req).then(res => {
                if (res.result) addGooglePayButton();
                else showError("Google Pay not available.");
            })
        ...
        client.loadPaymentData(getPaymentDataRequest())
            ...
            .catch(err => {
                if (err && err.statusCode !== "CANCELED") {
                    showError("Google Pay error: " + (err.statusMessage || err.statusCode || "unknown"));
                }
            });
    }
            ...
            $.post(POST_URL, {
                ...
                try {
                    ...
                    if (r.ok && r.redirect) {
                        ...
                        showError(r.error || "Authorisation failed");
                    }
                } catch (e) {
                    showError("Unexpected server response");
                }
            }).fail(function(jq) {
                showError("Network error: " + (jq.responseText || jq.status));
            });
        } catch (e) {
