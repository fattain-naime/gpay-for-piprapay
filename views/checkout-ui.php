<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gpay_token'])) {
    ...
    $payment_id = intval($_POST['payment_id'] ?? 0); $token = $_POST['gpay_token'] ?? ''; $mode = isset($settings['gpay_mode']) ? strtolower($settings['gpay_mode']) : 'sandbox';
    if (!$payment_id || !$token) { echo json_encode(['ok'=>false,'error'=>'Missing payment_id or token']); exit; }
    ...
    if ($mode !== 'live') {
      ...
      if (function_exists('pp_set_transaction_byid') && pp_set_transaction_byid($payment_id, $plugin_slug, $plugin_info['plugin_name'], 'Sandbox User', $auth_id, 'completed', 'Sandbox simulated authorisation: '.$auth_id)) {
        echo json_encode(['ok'=>true,'redirect'=>pp_get_paymentlink($payment_id)]); exit;
      } else { echo json_encode(['ok'=>false,'error'=>'Failed to mark completed in sandbox']); exit; }
    } else {
      ...
      if (file_exists($stub)) {
        ...
        try {
          ...
          echo json_encode(['ok'=>false,'error'=>'Decrypt stub failed: '.$e->getMessage()]); exit;
        }
      }
      echo json_encode(['ok'=>false,'error'=>'Decrypt stub not found']); exit;
    }
  }

  $transaction_details = pp_get_transation($payment_id);
  $setting = pp_get_settings();
  $plugin_slug = 'gpay-direct'; $plugin_info = pp_get_plugin_info($plugin_slug);
...
<!doctype html><html><head>
  ...
  <style>
    :root{
      --text: <?php echo $global_text?>;
      --btn-bg: <?php echo $primary_btn?>;
      --btn-text: <?php echo $btn_text?>;
      --card: #ffffff; --muted:#f8f9fa; --border:#e9ecef;
    }
    body{font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial; background:#f5f6f8; color:#222;}
    .wrap{max-width:900px;margin:2rem auto;padding:0 1rem}
    .cardx{background:#fff;border:1px solid var(--border);border-radius:16px;box-shadow:0 10px 30px rgba(0,0,0,.05);overflow:hidden;}
    .head{display:flex;align-items:center;gap:.75rem;padding:1rem 1.25rem;background:var(--muted);border-bottom:1px solid var(--border);}
    .head .back{cursor:pointer;display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:8px;background:#fff;border:1px solid var(--border);}
    .head .logo{width:32px;height:32px;border-radius:6px;object-fit:cover;margin-left:.5rem;background:#fff}
    .title{font-weight:600;font-size:1rem;color:#333;margin:0}
    .sublabel{font-size:.875rem;color:#6c757d}
    .body{padding:1.25rem}
    .grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}
    .amountBox{display:flex;align-items:center;gap:1rem;background:var(--muted);border:1px solid var(--border);border-radius:12px;padding:1rem}
    .brand{width:48px;height:48px;border-radius:10px;background:#fff;display:grid;place-items:center;border:1px solid var(--border)}
    .value{font-size:1.6rem;font-weight:800;color:var(--text)}
    table.summary{width:100%;margin-top:8px}
    table.summary td{padding:.35rem 0;color:#495057}
    table.summary td:last-child{text-align:right;font-weight:600;color:#111}
    .rightCol{
  display:flex;
...
#gpay-button-container > div{
  width:100% !important;     /* make Google button span full width */
  max-width:400px;           /* optional: limit width */
  display:flex;
  justify-content:center;
}    .btn-primary{background:var(--btn-bg);border-color:var(--btn-bg);color:var(--btn-text);width:100%}
    .helper{font-size:.875rem;color:#6c757d}
    .footer{padding:1rem;border-top:1px solid var(--border);text-align:center;font-size:.875rem;color:#6c757d}
    #gpay-error{margin-top:12px}
    ...
    <div class="cardx">
      <div class="head">
        <div class="back" onclick="location.href='<?php echo $back_link?>'">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M15 18l-6-6 6-6" stroke="#333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </div>
      ...
      <div class="body">
        <div class="grid">
          <div>
            <div class="amountBox">
              <div class="brand"><img src="<?php echo pp_get_site_url().'/pp-content/plugins/'.$plugin_info['plugin_dir'].'/'.$plugin_slug.'/assets/icon.png';?>" alt="" style="height:26px"></div>
              <div>
            ...
            <table class="summary">
              <tr><td>Amount</td><td><?php echo number_format($transaction_amount,2).' '.$settings['currency']?></td></tr>
              <tr><td>Fee (fixed)</td><td><?php echo number_format($fee_fixed,2).' '.$settings['currency']?></td></tr>
              <tr><td>Fee (<?php echo number_format($fee_percent,2)?>%)</td><td><?php echo number_format(($grand_total-$transaction_amount-$fee_fixed),2).' '.$settings['currency']?></td></tr>
              <tr><td><strong>Grand total</strong></td><td><strong><?php echo number_format($grand_total,2).' '.$settings['currency']?></strong></td></tr>
            </table>
          </div>

          <div class="rightCol">
            <?php if(empty($publicKey)): ?>
              <div class="alert alert-danger">Google Pay DIRECT requires an ECv2 Public Key. Add it in gateway settings.</div>
            <?php else: ?>
      ...
      <div class="footer">Powered by <a href="https://piprapay.com/" target="_blank" style="color:var(--text);text-decoration:none"><strong>PipraPay</strong></a></div>
    </div>
  </div>
